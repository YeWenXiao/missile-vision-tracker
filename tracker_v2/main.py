...
def main():
    ...
    # 录像
    save_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'tracker_v2_output.avi')
    video_writer = None
    ...
    try:
        while True:
            ...
            if state == STATE_SEARCH:
                ...
                if feature_bank.is_loaded() and detections:
                    # 有照片集 → YOLO+特征匹配
                ...
                # 无YOLO检测时，纯模板匹配
                if found_det is None and feature_bank.is_loaded():
                    tmpl_result = matcher.template_search(frame)
                    if tmpl_result:
                        x1, y1, x2, y2, score = tmpl_result
                        found_det = (x1, y1, x2, y2, score, 0)
                        found_score = score
                ...
                if found_det is not None:
                    ...
                    pitch_pid.reset()
                    state = STATE_TRACK
                    print(f'[搜索→追踪] 找到目标! conf:{track_conf:.2f} match:{found_score:.2f}')
                ...
                if matched_det:
                    ...
                    if (feature_bank.is_loaded() and
                        ...
                        if crop.size > 0:
                            ...
                            if not verified:
                                print(f'[追踪] 特征验证失败! 可能跟错目标')
                                # 降低匹配分，但不立即切状态

                    # 检查是否进入末端阶段
                    if box_ratio > TERMINAL_BOX_RATIO:
                        state = STATE_TERMINAL
                        print(f'[追踪→末端] 目标占画面 {box_ratio:.0%}，进入末端锁定')
